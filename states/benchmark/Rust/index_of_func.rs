...
fn index_of(array :String, symbol :char) -> Result<usize, String> {
    if array.len() == 0 {
        return Err ("IncorrectInput".to_string());
    }

    for index in 0..array.len() {
        if array.as_bytes()[index] as char == symbol {
             return Ok(index);
        }
    }
...
fn run() {
    ...
    let array :Vec<String> = vec!["0".to_string(), "".to_string(), "a".to_string(), "abc".to_string(), "qwertyuiopasdfghjklzxcvbnm".to_string() ];
    let symbols : Vec<char> = vec!['a', '0', '=' ];

    let sarr : usize = array.len();
    let ssym : usize = symbols.len();
    for iarr in 0..sarr {
        for isym in 0..ssym {
            //let symbol :char = 
            let result = index_of(array[iarr as usize].clone(), symbols[isym as usize] as char);
            match result {
                Ok(val)=>{
                    // something
                },
...
fn main() {
	println!("Start \"index_of\" main()\n");
	
	let mut test :Test = Test::new(10 as i32, 1000000 as i32, String::from("index_of")); 
	//test.testCount = 10;
